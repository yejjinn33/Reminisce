...
public class Cat_moves1 : MonoBehaviour
{
    ...
    public float original_speed;     // 원래 속도 저장
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        // 바나나 태그 처리
        if (collision.gameObject.CompareTag("Banana") && !is_slowed)
        {
            Debug.Log($"Collided with Banana: {collision.gameObject.name}");
    ...
    // 고양이가 길고양이와 부딪혔을 때 속도 감소
    public IEnumerator SlowDown()
    {
        if (slowDownCoroutine != null)
        {
            StopCoroutine(slowDownCoroutine); // 기존 코루틴 중지
        }

        slowDownCoroutine = StartCoroutine(SlowDownCoroutine());  // 코루틴 시작
        yield return slowDownCoroutine;  // SlowDownCoroutine이 완료될 때까지 기다림
    }

    private IEnumerator SlowDownCoroutine()
    {
        is_slowed = true;
        cat_speed *= 0.11f; // 속도 감소
        Debug.Log("cat"+cat_speed);
